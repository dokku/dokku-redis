...
An image other than redis has no version to fall back on, because the version this plugin pins belongs to redis, so name one alongside it.

```shell
dokku redis:create lollipop --image <image> --image-version <version>
```
...
```shell
dokku redis:create lollipop --log-opt max-size=20m,max-file=3
```

The container is restarted by docker whenever it stops, which a service may change for itself.

```shell
dokku redis:create lollipop --restart unless-stopped
```
...
```shell
dokku redis:create lollipop --volume /var/lib/dokku/data/storage/lollipop:/opt/extra:ro
```
...
```shell
# usage
dokku redis:info [<service>] [--info-flags...]
```
...
```shell
dokku redis:set lollipop initial-network my-network
```
...
It is possible to change the protocol for `REDIS_URL` by setting the environment variable `REDIS_DATABASE_SCHEME` on the app. Doing so after linking means unlink no longer finds the variable it set, and leaves it in place, so we advise you to unlink before proceeding.
...
An app is still linked after its `REDIS_URL` is changed to point elsewhere, and is unlinked the same way. The variable it now holds is not the service's, so it is left alone, nothing is unset, the app is not restarted, and a warning says so.
...
```shell
dokku redis:set lollipop log-opt max-size=20m,max-file=3
```
...
> NOTE: a log setting or a restart policy reaches the container the next time one is built. redis:restart keeps the container it has, so use redis:stop and then redis:start on a service that is already running.

### mount a host path or docker volume into the service container

```shell
# usage
dokku redis:mount [--replace] <service> <source:container-dir[:options]>...
```
...
```shell
dokku redis:mount lollipop /var/lib/dokku/data/storage/lollipop:/opt/extra
```
...
```shell
dokku redis:mount lollipop /var/lib/dokku/data/storage/lollipop:/opt/extra:ro,z
```

The same can be said with flags instead:

```shell
dokku redis:mount lollipop /var/lib/dokku/data/storage/lollipop:/opt/extra --volume-readonly --volume-options z
```
...
```shell
dokku redis:mount --replace lollipop /srv/a:/opt/a:ro /srv/b:/opt/b
```
...
```shell
# usage
dokku redis:unmount [--all] <service> [<source:container-dir>...]
```
...
```shell
dokku redis:unmount lollipop /var/lib/dokku/data/storage/lollipop:/opt/extra
```

Remove every mount the service has:

```shell
dokku redis:unmount --all lollipop
```
...
```shell
dokku redis:upgrade lollipop --image-version 1.2.3
```
...
```shell
dokku redis:upgrade lollipop --volume /var/lib/dokku/data/storage/lollipop:/opt/extra:ro
```
...
```shell
# usage
dokku redis:backup-schedule <service> <schedule> <bucket-name> [-u|--use-iam]
```
